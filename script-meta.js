module.exports = {
  name: 'Arca Refresher',
  namespace: 'LeKAKiD',
  author: 'LeKAKiD',
  description: 'Arca Live Extension',
  icon: 'https://github.com/lekakid/ArcaRefresher/blob/develop/icon.png?raw=true',
  downloadURL:
    'https://github.com/lekakid/ArcaRefresher/releases/latest/download/ArcaRefresher.user.js',
  updateURL:
    'https://github.com/lekakid/ArcaRefresher/releases/latest/download/ArcaRefresher.meta.js',
  homepageURL: 'https://github.com/lekakid/ArcaRefresher',
  supportURL: 'https://arca.live/b/namurefresher',
  match: ['https://*.arca.live/*'],
  exclude: ['https://*.arca.live/api/*'],
  connect: ['namu.la', 'saucenao.com', 'ascii2d.net'],
  noframes: true,
  'run-at': 'document-body',
  grant: [
    'GM_info',
    'GM_openInTab',
    'GM_setValue',
    'GM_getValue',
    'GM_deleteValue',
    'GM_listValues',
    'GM_xmlhttpRequest',
    'unsafeWindow',
  ],
};
